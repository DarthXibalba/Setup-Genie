...
mapfile -t valid_sections < <(get_sections "$config_file")
...
for section in "$@"; do
    mapfile -t required_scripts < <(read_config "$config_file" "$section" "REQUIRED")
    mapfile -t optional_scripts < <(read_config "$config_file" "$section" "OPTIONAL")
