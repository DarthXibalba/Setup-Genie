...
# SHA-256 checksum. Supports x86-64 and ARM64 on Ubuntu.
#
# Commands installed:
...
if [ ! -x "$apt_get_install" ]; then
    log_error "apt-get install helper is missing or not executable: $apt_get_install"
    exit 1
fi
...
if [[ ! -e "$install_dir" && ! -L "$install_dir" ]]; then
    ...
    "$apt_get_install" ca-certificates curl tar xz-utils coreutils libstdc++6
